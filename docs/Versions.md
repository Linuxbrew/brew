...
If there is a formula that currently exists in the core repo or has existed in the past (but has been migrated or otherwise deleted), you can recover it with the `brew extract` command to place a desired version of the formula in a custom tap. For example, if your project depends on automake 1.12 instead of the most recent version, you can obtain the last version of the formula for automake 1.12 by running `brew extract automake user/repo --version=1.12`. Keep in mind that formula obtained this way may have been written to a different syntax or standard than what exists now (e.g. checksums may have `sha1` checksums instead of `sha256`); the `brew extract` command does not edit or update formulae to meet current standards and style requirements.
